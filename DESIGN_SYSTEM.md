...
|---|---|---|---|
| Sidebar | Left, collapsible **46→164px** | Left, collapsible **46→164px** | Hidden |
| Bottom nav | — | — | Fixed bottom, 5 tabs |
| Content | 2-col grid | 2-col grid | 1-col stack |

**Утверждённые макеты (июнь 2026):**
- Desktop: `mwl_dark_theme.html` (оба theme, RU/HE)
- Tablet: `mwl_dark_tablet.html` (оба theme, RU/HE)
- Mobile: `mwl_dark_mobile.html` (оба theme, RU/HE)
...
Состав строго: **Logo + Brand + Search + Lang + Bell + User**
...
**Mobile bottom nav (5 tabs, без Редактора):**
Главная | Модули | Формулы | Лаборатория | Настройки
...
Nav item states:
```
active (both themes): color #22D3EE — icon + label, no background fill
hover  (both themes): color #94a3b8 — icon + label, no background fill
```
...
- Состояние collapsed/expanded сохраняется в `localStorage`
...
### §10b. Gap-анализ: макет ↔ Next.js реализация

*(Перенесено из `DASHBOARD_MENU_GAP_2026-06-24.md`, июнь 2026)*

Сравнение: макет `mwl_dark_theme.html` ↔ `frontend/app/dashboard/page.tsx` + `Header.tsx` + `Sidebar.tsx`

#### Статус реализации

| # | Задача | Статус |
|---|---|---|
| 8.1 | Header со встроенным search подключён в dashboard shell | [x] |
| 8.2 | Sidebar: добавлен «Редактор» + подменю (Граф/Атом/Группы/Экзамены) | [x] |
| 8.3 | Поведение §10a реализовано (open/close rules, auto-expand) | [x] |
| 8.4 | Nav states: color-only (без фоновых плашек), стабильные иконки | [x] |
| 8.5 | Persistence: `sidebarOpen` в localStorage + восстановление при монтировании | [x] |
| 8.6 | i18n ключи добавлены в `ru.json` / `he.json` | [x] |
| 8.7 | Роутинг: onClick пунктов меню подключён к маршрутизации | [ ] |
| 8.8 | Smoke-check: desktop/tablet/mobile × RU/HE × dark/light | [ ] |

#### Известные расхождения на момент анализа (до исправления)

- Header search отсутствовал в рабочем dashboard shell → исправлено [x]
- Sidebar не содержал «Редактор» и подменю → исправлено [x]
- Nav states имели фоновые плашки (не color-only) → исправлено [x]
- Collapse не сохранялся в localStorage → исправлено [x]
- `DashboardHeader.tsx` существовал, но не был подключён → исправлено [x]
...
**Редактор и его подменю недоступны на мобильном** — экран телефона не позволяет работать с конструктором контента.
